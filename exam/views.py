...
@login_required(login_url='adminlogin')
def admin_exam_setting(request):
    dict={
    'total_student':SMODEL.Student.objects.all().count(),
    'total_teacher':TMODEL.Teacher.objects.all().filter(status=True).count(),
    'total_course':models.Course.objects.all().count(),
    ...
    return render(request,'exam/admin_exam_setting.html',context=dict)
...
def admin_add_schedule_view(request):
    ...
    if request.method=='POST':
        ...
        if questionForm.is_valid():
            ...
            print("form is invalid")
        return HttpResponseRedirect('/admin-view-question')
    return render(request,'exam/admin_add_schedule.html',{'questionForm':questionForm})
...
def cam(request):
    return render(request, "exam/cam.html")
